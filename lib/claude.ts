...
2. REPLACEMENT / SWAP: "Replace [existing object, concretely described] with [new object: exact type, material, color, finish]. The new object occupies the same position, scale and perspective as the original." + preservation clause.
   CRITICAL — when the new object is SIMILAR to the existing one (same category, e.g. baseboard→baseboard, door→door), the prompt MUST lead with the concrete DIFFERENCES versus what is currently in the image ("noticeably taller than the current low baseboard", "a flat face instead of the current rounded profile", "a wider, more pronounced light gap at the very bottom"). Confirmed failure: a prompt that describes the replacement generically ("white baseboard with a warm glow") when the image already contains a white glowing baseboard produces NO visible change — the model sees its job as already done. Compare the current object with the requested one attribute by attribute and spell out every difference; if you cannot find any meaningful difference, say so in the "summary" instead of writing a no-op prompt.
...
- CRITICAL — GROUND the prompt in the reference. When the user says to use "my X" / "the one from the reference", study the reference photo and extract its concrete, distinguishing visual attributes: proportions (height relative to nearby elements), profile shape (flat / rounded / stepped), edge and groove details, where any light gap sits, color and sheen. State those attributes explicitly in the prompt, phrased as DIFFERENCES from the object currently in the main image (per the replacement rule above). Never write a generic description that could equally describe the object already in the photo — that produces no visible change. If the reference photo is too tight a close-up to reveal such attributes (no context, just a texture), still describe whatever is genuinely visible (band layout, proportions, finish) and note the uncertainty in the "summary" so the user knows a more informative reference photo would help.
